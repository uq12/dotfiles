...
complete -c $PROG -f -n '__fish_gopass_uses_command completion' -a bash -d 'Подкоманда: Источник для автоматического завершения в bash'
...
complete -c $PROG -f -n '__fish_gopass_uses_command mounts remove -l version -d "распечатать версию"'
complete -c $PROG -f -n '__fish_gopass_uses_command mounts' -a versions -d 'Подкоманда: Показать версии провайдера монтирования'
...
complete -c $PROG -f -n '__fish_gopass_needs_command' -a help -d 'Команда: показывает список команд или справку для одной команды.'
